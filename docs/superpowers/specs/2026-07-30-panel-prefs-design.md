...
- ~~복원이 늦게 `'summary'`로 전환하면 기존 탭-전환 효과가 최상단 스크롤을 수행 —
  의도된 동작(Summary는 항상 최상단), 추가 배선 불필요.~~ **정정 (2026-07-31, 최종
  리뷰 I1):** 이 전제가 틀렸다. 마운트 시점의 `scrollIntoView(tabSection)`은 "최상단"이
  아니라 헤더(VideoCard·자막 표시·버튼·스테퍼)를 지나 ~300px **아래로** 내려가는
  러치다. 복원 전환은 `restoringTabRef`로 표시하고, 탭-전환 스크롤 효과가 이를
  check-and-clear로 1회 스킵한다 — 패널은 열린 그대로(최상단) Summary 탭이 선택된
  상태로 페인트된다. 실제 사용자 클릭의 스크롤 동작은 무변경.
