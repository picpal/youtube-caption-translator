...
- **재개 일관성**: SW evict 후 재개(auto-resume)되는 미완 작업은 현재 설정이 아니라
  **기존 레코드에 스탬프된 targetLang으로 완주**한다(없으면 'ko') — 재개 사이에 설정이
  바뀌어도 혼합 언어 세그먼트가 생기지 않는다.
- **언어 불일치 재생성 = 전체 신선 시작**: 작업의 targetLang ≠ 기존 레코드의
  targetLang(?? 'ko')이면 파이프라인은 기존 레코드의 glossary·완료 배치를 일절
  재사용하지 않고 처음부터 시작한다 — 이전 언어의 용어집/부분 번역이 새 언어 결과를
  오염시키는 것을 막는다.
