...
**모델**: `gemini-3.5-flash-lite`(기존 `src/lib/gemini.ts` 재사용·확장; Task R5에서
`gemini-3.6-flash`로부터 교체 — 강제 thinking이 규칙 많은 번역 프롬프트에서 폭발해 50세그먼트
청크 1건에 150초 이상 소요·정체되는 것을 실측, `gemini-3.5-flash-lite`는 동일 조건에서 5.8초·
thinking 0으로 약 25배 빠름). 무료 티어 RPM/TPM 상한 → 동시성 상한 + 백오프로 준수. 배치 크기·
동시성은 상수로 두고 실측으로 조정.
