...
- [ ] **Step 3: Pipeline threading** — `runTranslationPipeline` params gain `targetLang: TargetLang`. Inside:
  ...
  - Deps calls `deps.analyzeGlossary(fullText, key)` / `deps.translateBatch(segs, glossary, key)` keep their shapes — background closes over the effective language when building deps? NO — the effective language is decided INSIDE the pipeline (resume rule), so the deps signatures must gain a `targetLang` last param instead: `analyzeGlossary: (fullText, key, targetLang)`, `translateBatch: (segs, glossary, key, targetLang)`, and the pipeline passes its effective local. Update the deps interface + bg injections + test fakes accordingly.
